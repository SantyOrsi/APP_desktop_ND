...
// Un dígito suelto en letras (para leer decimales de a uno, ej. "0,07" -> "cero siete")
const DIGITOS = ['cero', 'uno', 'dos', 'tres', 'cuatro', 'cinco', 'seis', 'siete', 'ocho', 'nueve'];

// Monto CON decimales en letras, en minúscula: "4530.94" -> "cuatro mil
// quinientos treinta y uno coma noventa y cuatro" / "0.07" -> "cero coma
// cero siete" (si los centavos son menores a 10, se leen dígito por
// dígito para no perder el cero adelante).
const montoALetrasConDecimales = (valor) => {
  const num = Number(valor);
  if (!Number.isFinite(num)) return '';
  const parteEntera = Math.floor(Math.abs(num));
  const parteDecimal = Math.round((Math.abs(num) - parteEntera) * 100);
  const enteraTexto = parteEntera === 0 ? 'cero' : numeroALetras(parteEntera).toLowerCase();
  if (parteDecimal === 0) return enteraTexto; // es un número entero, sin "coma cero cero"
  const decimalTexto = parteDecimal < 10
    ? `cero ${DIGITOS[parteDecimal]}`
    : convertirGrupo(parteDecimal);
  return `${enteraTexto} coma ${decimalTexto}`;
};

// Fecha en letras: "28/08/2026" -> "veintiocho de agosto de 2026"
const fechaEnLetras = (fechaStr) => {
  const f = parseFecha(fechaStr);
  if (!f.dia || !f.mes) return '';
  const diaTexto = convertirGrupo(Number(f.dia));
  return `${diaTexto} de ${f.mes} de ${f.anio}`;
};
...
async function generarContratoPDF(presupuesto, contrato, servicio = null, opciones = {}) {
  ...
  // El total usa la moneda del presupuesto; la seña y el saldo usan la
  // moneda propia del CONTRATO, que puede ser distinta (ej. total
  // cotizado en pesos, pero seña cobrada en dólares).
  const simboloMonedaTotal = presupuesto.moneda === 'USD' ? 'US$' : '$';
  const simboloMonedaSenia = contrato.moneda === 'USD' ? 'US$' : '$';
  ...
  if (incluirImporte) {
    const palabraMonedaTotal = presupuesto.moneda === 'USD' ? 'dólares' : 'pesos';
    const palabraMonedaSenia = contrato.moneda === 'USD' ? 'dólares' : 'pesos';
    const seniaLetras = contrato.senia ? montoALetrasConDecimales(contrato.senia) : '';
    const saldoLetras = contrato.saldo ? montoALetrasConDecimales(contrato.saldo) : '';
    const fechaCancLetras = fechaEnLetras(contrato.fechaCancelacion);

    y = dibujarParrafo(page, fontNormal, fontBold, [
      normal('Quinto:'), normal(`El importe del viaje será de ${simboloMonedaTotal}`), dato(montoViaje), normal(palabraMonedaTotal), dato(numeroALetras(montoViaje)),
      normal(`. Final, el que será abonado de la siguiente manera: en concepto de seña ${simboloMonedaSenia}`), dato(contrato.senia),
      normal(palabraMonedaSenia), dato(seniaLetras), normal(palabraMonedaSenia),
      normal(`y el saldo ${simboloMonedaSenia}`), dato(contrato.saldo), normal(palabraMonedaSenia), dato(saldoLetras), normal(`${palabraMonedaSenia} ,`),
      normal('antes del día'), dato(fechaCancLetras),
      normal('.'), dato(contrato.metodoPago), normal('.'),
    ], y);
    y -= 7;
  }
  ...
  if (contrato.emitidoPor) {
    page.drawRectangle({ x: 380, y: 824, width: 190, height: 16, color: rgb(1, 1, 1), borderColor: rgb(0.8, 0.8, 0.8), borderWidth: 0.75 });
    page.drawText(`Emitido por: ${contrato.emitidoPor}`, { x: 386, y: 828, size: 8, font: fontNormal, color: NEGRO });
  }
